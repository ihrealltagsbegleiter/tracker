...
async function load(key, fallback) {
  try { if(!window.storage) return fallback; const r=await window.storage.get(key); return r?JSON.parse(r.value):fallback; } catch { return fallback; }
}
async function save(key, value) {
  try { if(!window.storage) return; await window.storage.set(key,JSON.stringify(value)); } catch(e) { console.error(e); }
}
...
function streakCount(logs, routineId) {
  ...
  // best streak (scan all keys sorted)
...
function computeCorrelations(logs, trackFields) {
  ...
  // sleep correlation
  const sleepPairs=entries.filter(e=>e.tracking["t1"]!=null).map(e=>[Number(e.tracking["t1"]),Number(e.tracking[LIFE_FIELD])]);
  // already covered by fields loop, skip duplicate
...
function MicroLine({ data, color="#c8f060", height=40 }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{display:"block",height}}>
      ...
      {data.map((v,i)=>v!=null&&(
        <circle key={i} cx={xs[i]} cy={ys[i]} r="2.5" fill={color} opacity="0.7"/>
      ))}
    </svg>
  );
}
...
function MealTracker({meals=[],onChange,suggestions}){
  ...
  const filteredSugg=useMemo(()=>{
    if(!search.trim()) return suggestions.slice(0,8);
  ...
  const commit=()=>{
    ...
    setDraft({time:nowTime(),items:"",feel:null,notes:""}); setOpen(false); setSearch(""); setShowSuggest(false);
  };
  ...
  const cancel=()=>{ setDraft({time:nowTime(),items:"",feel:null,notes:""}); setEditId(null); setOpen(false); setSearch(""); setShowSuggest(false); };
          ...
          <div className="meal-form-row">
            ...
            <div className="meal-form-group" style={{flex:1,position:"relative"}}>
              ...
              <input className="edit-input" placeholder="z.B. Haferflocken, Banane, Kaffee"
                value={draft.items}
                autoFocus
                onChange={e=>{ setDraft({...draft,items:e.target.value}); setSearch(e.target.value); setShowSuggest(true); }}
              ...
              {showSuggest&&filteredSugg.length>0&&(
                <div className="suggest-list">
                  {filteredSugg.map((s,i)=>(
                    <div key={i} className="suggest-item" onMouseDown={()=>{ setDraft({...draft,items:s}); setShowSuggest(false); }}>
                      {s}
                    </div>
                  ))}
                </div>
            ...
            <div className="feel-pills">
              {FEEL.map(o=>(
                <button key={o.v} className={`feel-pill ${draft.feel===o.v?"selected":""}`}
                  ...
                  onClick={()=>setDraft({...draft,feel:draft.feel===o.v?null:o.v})}>
                  {o.label}
                </button>
              ))}
            </div>
...
export default function App() {
  ...
  const [trackFields,setTrackFields]=useState(DEFAULT_FIELDS);
  const [logs,setLogs]=useState({});
  const [settings,setSettings]=useState({theme:"dark",statRange:30});
  ...
  useEffect(()=>{
    (async()=>{
      const [r,l,t,s]=await Promise.all([
        load(KEYS.routines,DEFAULT_ROUTINES),
        load(KEYS.logs,{}),
        load(KEYS.trackFields,DEFAULT_FIELDS),
        load(KEYS.settings,{theme:"dark",statRange:30}),
      ]);
      setRoutines(r); setLogs(l); setTrackFields(t); setSettings(s); setLoaded(true);
    })();
  },[]);

  // Persist
  useEffect(()=>{ if(loaded) save(KEYS.routines,routines); },[routines,loaded]);
  useEffect(()=>{ if(loaded) save(KEYS.logs,logs); },[logs,loaded]);
  useEffect(()=>{ if(loaded) save(KEYS.trackFields,trackFields); },[trackFields,loaded]);
  useEffect(()=>{ if(loaded) save(KEYS.settings,settings); },[settings,loaded]);
    ...
    notifRef.current=setInterval(()=>{
      ...
      routines.filter(r=>r.active&&r.time===now).forEach(r=>{
        if(!logs[today]?.routines?.[r.id]){
          new Notification("Alltagsbegleiter",{body:`Zeit für: ${r.name}`,icon:""});
        }
      });
    },30000);
  ...
  // Meal autocomplete suggestions from history
  const mealSuggestions=useMemo(()=>{
    const freq={};
    Object.values(logs).forEach(d=>(d.meals||[]).forEach(m=>{
      if(m.items) freq[m.items]=(freq[m.items]||0)+1;
    }));
  ...
  // Meal history (last 14 days with entries)
  const mealHistory=getPastKeys(14).reverse().map(k=>({date:k,meals:logs[k]?.meals??[]})).filter(d=>d.meals.length>0);

  const requestNotif=async()=>{
    if("Notification" in window){ const p=await Notification.requestPermission(); setShowNotifHint(p!=="granted"); }
  };

  const doExportJSON=()=>{
    const blob=new Blob([JSON.stringify({logs,routines,trackFields},null,2)],{type:"application/json"});
    const a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(blob),download:`alltagsbegleiter-${today}.json`}); a.click();
  };
  const doExportCSV=()=>{
    const csv=exportCSV(logs,trackFields);
    const blob=new Blob([csv],{type:"text/csv;charset=utf-8;"});
    const a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(blob),download:`alltagsbegleiter-${today}.csv`}); a.click();
  };
      ...
      <main className="main">
            ...
            <div className="progress-card">
              <svg className="ring" viewBox="0 0 120 120">
                ...
                <circle cx="60" cy="60" r="52" className="ring-fg"
                  strokeDasharray={`${327*progress} ${327*(1-progress)}`}/>
              </svg>
              ...
              {activeRoutines.map(r=>{
                ...
                return(
                  <RoutineItem key={r.id} routine={r}
                    done={!!todayLog.routines[r.id]}
                    onToggle={()=>toggleRoutine(r.id)}
                    streak={streakCount(logs,r.id)}
                    weekDots={wdots}/>
                );
              })}
            </div>
          ...
          <div className="section-stack">
            <div className="card">
              <div className="card-head">
                Mahlzeiten heute
                <span className="card-sub">{(todayLog.meals??[]).length} Einträge</span>
              </div>
        ...
        {tab==="statistik"&&(
          <div className="section-stack">
            ...
            <div className="card">
              <div className="card-head">
                Korrelation mit Lebensgefühl
                <span className="card-sub"> (Pearson r, min. 3 Einträge)</span>
              </div>
              {correlations.length===0
                ...
                :correlations.map(c=>(
                  <div key={c.label} className="corr-row">
                    ...
                    <div className="corr-bar-outer">
                      <div className={`corr-bar-inner ${Number(c.r)>0?"pos":"neg"}`} style={{width:`${Math.abs(Number(c.r))*100}%`}}/>
                    </div>
              ...
              <p style={{fontSize:9,color:"var(--muted)",marginTop:12,lineHeight:1.6}}>
                r=1.0 perfekte positive Korrelation · r=-1.0 perfekte negative Korrelation · r=0 kein Zusammenhang
              </p>
            </div>
          </div>
        )}
        ...
        {tab==="einstellungen"&&(
          <div className="section-stack">
            <div className="card">
              ...
              <p className="settings-hint">Benachrichtigungen werden zur Routine-Uhrzeit ausgelöst solange diese Seite geöffnet ist. Alle 30 Sekunden wird geprüft ob eine Routine fällig ist.</p>
              <button className="btn-primary" onClick={requestNotif}>Benachrichtigungen aktivieren</button>
              {showNotifHint&&<p className="warn">Zugriff verweigert – bitte in den Browser-Einstellungen erlauben.</p>}
            </div>
              ...
              {editRoutine&&(
                <div className="edit-form">
                  ...
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <input type="number" className="edit-input" min={1} max={7} value={editRoutine.weeklyGoal??7}
                      onChange={e=>setEditRoutine({...editRoutine,weeklyGoal:Number(e.target.value)})} style={{width:60}}/>
            ...
            <div className="card">
              ...
              <p className="settings-hint" style={{marginTop:10}}>CSV öffnet sich direkt in Excel. Alle Tracking-Werte und Mahlzeiten-Anzahl pro Tag.</p>
            </div>
          </div>
        )}
      </main>
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .app{
    ...
    --text:#e8e4dc;--muted:#6b6b6b;--danger:#f06060;
    --radius:2px;
  ...
  .header{border-bottom:1px solid var(--border);padding:20px 24px 16px;position:sticky;top:0;background:var(--bg);z-index:10;}
  ...
  .loading{display:flex;justify-content:center;align-items:center;height:100vh;gap:8px;}
  .loading-dot{width:6px;height:6px;border-radius:50%;background:var(--accent);animation:pulse 0.8s infinite;}
